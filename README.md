...
> **参考教程**：鸣潮体力监控功能的实现参考了 [此教程](https://xn--ntsp09fr3m.com/kurobbs.html)。感谢原作者。
...
因此，在使用脚本前，您需要先对每个自动化程序进行设置和调试（如保证战斗类任务中队伍携带生存位以维持续航），保证它们都能正常地完成每日任务，并且在完成后自动关闭以避免干扰下一个脚本的执行。
...
*   3. **March7thAssistant (Honkai: Star Rail)**：
    *   `M9A` 并没有提供基于命令行的参数，但默认在通过命令行方式启动时会执行”完整运行“任务。推荐只保留执行”日常“任务，关闭其他任务的执行，并在”程序“中设置”任务完成后：退出“。
*   4. **ok-ww (Wuthering Waves)**：`task_4_okww.bat`
...
*   **对于任务 1-4**：右键点击 `task_*.bat` 文件，选择“编辑”，修改 `APP_DIR` 和 `APP_EXE` 为你的实际路径。
*   **对于任务 5 (鸣潮监控)**：
    *   编辑 `ww_monitor.py`。
